...
export default function Car({
  ...
  description,
  userName,
}: CreateCarType & { userName: string }) {
  const [imageInd, setImageInd] = useState(0);
  return (
    <div className="relative w-full flex flex-col border-2 border-white/30 rounded-lg">
      <div className="flex flex-row items-end px-5 py-3">
        <div className="text-4xl">{title}</div>
        <div className="text-3xl text-white/30 ml-3">{userName}</div>
      </div>
      <div
        className="h-64 bg-cover bg-center rounded-b-lg"
        style={{
          backgroundImage: `url(${images[imageInd]})`,
        }}
      >
          ...
          <div className="bg-black/80 w-full h-full flex flex-row items-center px-4">
            ...
            <div className="ml-5 md:max-w-[300px] sm:max-w-[300px] max-w-[200px]">
              <div className="flex flex-row flex-wrap gap-2">
                {tags.map((t, index) => {
                  return <Tag key={index} content={t} />;
                })}
              </div>
